...
int memoid = 0;
 ...
 int fight_re;
 int fight_re1;
 int score_gameover;				//スコア
 int sear_cor;						//探索済みを確認
 int escape_num;					//脱出番号
 int escape_item;					//脱出item
 int escape_person;					//脱出person
 int game_num;						//gameover判定
 int move_search;						//訪問or探索or戦闘
 int act_sel;						//選択肢入力の変数
 int wp1;							//武器の選択
 int at1;							//戦闘選択
 int lose_point;					//敗北回数
 int re;							//戦闘結果
 int boss_1 = 1;
int boss_2;				//boss 条件アイテム
...
struct score_save scoreboard[100] = {
	0,"",1,
	0,"",2,
	0,"",3,
	0,"",4,
...
struct map data[100] = {	//マップ構造体の初期化
	00, "旧研究棟", "0", "アラクネ", "0", "0"
...
struct combination item[100] = {	//キーアイテム構造体の初期化
	0, "空のライター", "ライターオイル", "中身があるライター"
	, 1, "毒針", "枝", "毒枝"
	, 2, "注射器", "抗体", "ワクチン"
};

struct player stock[100] = {		//プレイヤー構造体の初期化
	0, "ハンマー", ""
	, 1, "サブマシンガン", ""
	, 2, "コンバットナイフ", "" };

const struct map reset_m[100] = {	//マップ構造体の初期化
	00, "旧研究棟", "0", "アラクネ", "0", "0"
	, 01, "旧研究棟", "研究資料1", "ゾンビコロニー", "0", "0"
	, 02, "隔離研究棟", "0", "ゾンビプラント", "0", "0"
	, 03, "隔離研究棟*第二シェルター", "0", "0", "同僚", "0"
...
const struct player reset_p[100] = {		//プレイヤー構造体の初期化
	0, "ハンマー", ""
	, 1, "サブマシンガン", ""
	, 2, "コンバットナイフ", "" };

struct memochou save[100] = {
	0,"1","1",
	1,"1","1",
	2,"1","1",
	3,"1","1",
...
const struct memochou reset_me[100] = {
	0,"1","1",
	1,"1","1",
	2,"1","1",
	3,"1","1",
...
int main(void) {
	while (game) {
		memcpy(&data, &reset_m, sizeof(reset_m));
		memcpy(&stock, &reset_p, sizeof(reset_p));
		pp = 0;
		memcpy(&save, &reset_me, sizeof(reset_me));
		esc1 = 1;
		same = 0;
		...
		while (play) {
			...
			for (j = 0; j < 100; j++) {
				if (mass < 31) {
					...
					if (act_sel == 1) {
						...
						for (mat1 = 0; mat1 < 50; mat1++) {		//材料アイテムが2つあるときキーアイテムに組み合わせする。
							for (mat2 = 0; mat2 < 50; mat2++) {
								for (mat3 = 0; mat3 < 3; mat3++) {
									if (!strcmp(stock[mat1].item_name, item[mat3].material1)) {
										if (!strcmp(stock[mat2].item_name, item[mat3].material2)) {
											printf("\n%sと%sを組み合わせて%sを獲得した。", stock[mat1].item_name, stock[mat2].item_name, item[mat3].combination);;
											//	printf("mat1:%s", stock[mat1].item_name);
											//	printf("mat2:%s", stock[mat2].item_name);
											if (mat1<mat2) {
						...
						//combination(mat1, mat2, mat3, stock, item);					//アイテムの組み合わせ
			...
			//	gameover_wl(0, score_gameover);	//敗北の結果表示
		...
		scoreboard[6].score = score_gameover;
		...
		if (scoreboard[6].score != 0) {
			if (score_gameover != 0) {
				...
				posxy(70, 7); scanf_s("%s", &scoreboard[6].name,sizeof(scoreboard[6].name));
				c = getchar();

				for (s1 = 5; s1 > -1; s1--) {
					s2 = s1 + 1;
					if (scoreboard[s1].score < scoreboard[s2].score) {
...
int act_select() {					//選択肢（何をやるか）の入力
	...
	//scanf_s("%d", &act_sel, sizeof(&act_sel));			//選択肢入力
...
int gameover_wl(int game_num, int score_gameover) { //勝利結果と敗北結果の表示
	...
	else if (game_num == 1) {	 //軽飛行機で脱出したときの勝利結果の表示
		print_win();
	...
	else if (game_num == 2) { //ボートで脱出したときの勝利結果の表示
		print_win();
	...
	else if (game_num == 3) { //クルーザで脱出したときの勝利結果の表示
		print_win();
	...
	else if (game_num == 4) { //クルーザで脱出+シオンしたときの勝利結果の表示
		print_win();
		posxy(62, 27);	printf("クルーザで脱出しました。\n");
			...
			if (em_id == 0) {
				...
				if (wp1 == 1) {//引き分け
					...
					item_dis(); memo();	//アイテムの表示
								...
								//, struct map data[100], struct player stock[100], struct monster enermy[100]
					posxy(POS_SX, POS_SY);
					printf("-------------------------------------------------------------------------------------------------------------------------------------------\n");
					printf("何をつかいますか。\n");
					printf("1.%s 2.%s 3.%s \n", stock[0].item_name, stock[1].item_name, stock[2].item_name);
...
int player_research() {
	//探索結果の内容
	//struct player stock[100], struct map data[100], struct monster enermy[100], struct combination item[100]
	int em_id;		//エナミーのid
	int key, key1;		//鍵のアイテムを探すための番号
	em_id = 6;
	int fight_re = 5;	//戦闘の結果
	int fight_num;
	move_search= 3;
				...
				if (strcmp(data[i].monster, "0")) {				//アラクネが倒されなかった場合は、
					...
					for (key = 0; key < 50; key++) {		//アラクネをあらわせるための条件
						if (!strcmp(stock[key].item_name, "中身があるライター")) {	//"中身があるライター"あって、
							...
							if (fight_re == 1) {
								strcpy_s(data[i].monster, sizeof(data[i].monster), "0");

								strcpy_s(stock[item_cor].item_name, sizeof(stock[item_cor].item_name), "隔離研究棟の鍵");//落としたアイテムを拾う。
								...
								check_place = 1;
								strcpy_s(data[i].item, sizeof(data[i].item), "1");
							}
						}
							...
							for (key1 = 0; key1 < 50; key1++) {
								if (!strcmp(stock[key1].item_name, "毒枝")) {	//"毒枝"があって、
									...
									if (fight_re == 1) {//◇戦闘勝利時\n");
										...
										strcpy_s(data[i].monster, sizeof(data[i].monster), "0");
										strcpy_s(stock[item_cor].item_name, sizeof(stock[item_cor].item_name), "クールザの鍵");	//落としたアイテムを拾う。
					...
					if (!strcmp(data[16].item, "1")) {
						if (!strcmp(data[4].item, "1")) {		//"軽飛行機の鍵"を拾って
							if (strcmp(data[16].person, "0")) {	//マップ上に社長があるとき、社長が仲間になる。
								...
								c = getchar();
								printf("%sが仲間になった。\n", data[16].person);
								strcpy_s(data[16].person, sizeof(data[16].person), "0");
							}
						}
					}
				}
...
void explan() {
	...
	posxy(POS_MX, POS_MY);
	printf("|①");
	posxy(POS_MX, POS_MY + 1);		 //マップの形を書く
	printf("| ┌ ┬ ┬ ┬ ┐   \n");
	posxy(POS_MX, POS_MY + 2);
	printf("| ├ ┼ ┼ ┼ ┤   \n");
	posxy(POS_MX, POS_MY + 3);
	...
	// posxy(POS_CX+20, POS_CY); printf("<Press the enter>\n");
				...
				if (strcmp(stock[4].item_name, "1")) {
					if (!strcmp(data[16].person, "0")) {
						if (mass < 31) {
							...
							game_num = 2;
							score_gameover = score_re();
							gameover_wl(1, score_gameover);
							mass = gameover();
							j = j + 150;
							play = 0;
							esc = 0;
...
void memo() {
	...
	for (mr = 0; mr < 9; mr++) {				//プレイヤーのアイテムを確認するためにプレイヤー構造体を検索して表示
			...
			mp = mr % 5 ;
				...
				printf("|");
					...
					printf("%s:%s", save[mr].name, save[mr].explan);
...
void inprint(char name[10], char explan[10]) {
	int ii;
	int mip=0;
	for (ii = pp; ii>-1; ii--) {
	...
	if(mip==0){
	strcpy_s(save[pp].name, sizeof(save[pp].name), name);
	strcpy_s(save[pp].explan, sizeof(save[pp].explan), explan);
	pp++;
	}
	mip = 0;
...
//		, 4, "アラクネ", "切", 2
	...
	if (!strcmp(data[i].monster, "ゾンビコロニー")) {
	...
	}else if (!strcmp(data[i].monster, "ゾンビ植物")) {
	...
	}else if (!strcmp(data[i].monster, "蟲ゾンビ")) {
	...
	}else if (!strcmp(data[i].monster, "ゾンビプラント")) {
		...
		}else if(!strcmp(data[i].monster, "アラクネ")){
